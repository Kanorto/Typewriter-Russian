...
- Understanding quest tracking and how it works
- Setting up automatic and manual quest tracking
- Using placeholders to show tracked quest information
- Creating sidebar displays for quest objectives
- Adding visual feedback for quest status changes
</Section>
<Section divider='right'>
## Quest Display Overview
Quest display in Typewriter uses a tracking system where only one quest is "tracked" at a time. This tracked quest becomes the focus for all display systems like scoreboards, sidebars, and custom UIs, keeping players focused on their current objective.
</Section>
</SplitLayout>
...
### How Tracking Works
When a quest is tracked, it becomes the "active" quest that players see in their quest displays. This tracked quest can then be referenced by other systems like scoreboards, action bars, or custom UIs.
...
- When a quest becomes active (based on its Active Criteria)
- When an objective becomes active and has a higher priority than the currently tracked quest
